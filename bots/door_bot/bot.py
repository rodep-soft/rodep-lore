...
# --- 環境変数読み込み ---
load_dotenv()
TOKEN = os.getenv("DOOR_BOT_TOKEN") or os.getenv("DISCORD_TOKEN")

try:
    # 互換性のため CHANNEL_ID にも対応しつつ、複数指定可能な CHANNEL_IDS を優先
    channel_ids_str = (
        os.getenv("DOOR_BOT_CHANNELS") or os.getenv("CHANNEL_IDS") or os.getenv("CHANNEL_ID", "")
    )
...
def speak_zunda(text, speaker_id=3, host="127.0.0.1", port=50021):
    ...
    try:
        ...
        query_res = requests.post(f"http://{host}:{port}/audio_query", params=params, timeout=3)
...
async def status(ctx):
    ...
    trigger_text = (
        last_triggered_time.strftime("%Y-%m-%d %H:%M:%S") if last_triggered_time else "なし"
    )
        ...
        if was_below_threshold:
            ...
            if today_date > last_sent_date or last_sent_date is None:
                ...
                if success_count > 0:
                    ...
                    print(
                        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Notification sent to {success_count} channels!"
                    )
