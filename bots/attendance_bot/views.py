...
class AttendanceView(View):
    ...
    async def update_attendance(self, interaction: discord.Interaction, status: str):
        ...
        await interaction.response.send_message(
            f"「{status}」で出欠を登録しました！", ephemeral=True
        )
...
class MeetingAnnouncementModal(Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        ) or await interaction.client.fetch_channel(self.target_channel_id)
        if channel:
            ...
            await interaction.followup.send(
                "✅ 定例会の告知メッセージを送信しました！", ephemeral=True
            )
        else:
            await interaction.followup.send(
                "❌ 告知先のチャンネルが見つかりませんでした。", ephemeral=True
            )
...
class LocationSelectView(View):
    ...
    @discord.ui.button(
        label="🏫 部室", style=discord.ButtonStyle.primary, custom_id="btn_loc_bushitsu"
    )
    async def btn_bushitsu(self, interaction: discord.Interaction, button: discord.ui.Button):
        modal = MeetingAnnouncementModal(
            self.meeting_info, self.target_channel_id, selected_location="部室"
        )
        await interaction.response.send_modal(modal)

    @discord.ui.button(
        label="🏢 研究棟", style=discord.ButtonStyle.success, custom_id="btn_loc_kenkyuto"
    )
    async def btn_kenkyuto(self, interaction: discord.Interaction, button: discord.ui.Button):
        modal = MeetingAnnouncementModal(
            self.meeting_info, self.target_channel_id, selected_location="研究棟"
        )
        await interaction.response.send_modal(modal)

    @discord.ui.button(
        label="✏️ その他・入力する", style=discord.ButtonStyle.secondary, custom_id="btn_loc_other"
    )
    async def btn_other(self, interaction: discord.Interaction, button: discord.ui.Button):
        modal = MeetingAnnouncementModal(
            self.meeting_info, self.target_channel_id, selected_location="指定の場所"
        )
        await interaction.response.send_modal(modal)
...
class MeetingPromptView(View):
    ...
    @discord.ui.button(
        ...
        custom_id="btn_create_meeting_announcement",
    )
    async def btn_create(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(
            "📍 **開催場所を選択してください:**", view=view, ephemeral=True
        )
