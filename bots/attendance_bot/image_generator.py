...
def create_summary_image(categories, is_saturday, today_str, is_test, display_order):
    ...
    height = (
        header_height
        + 60
        + (len(categories) * (category_padding + 60))
        + (total_users_lines * line_height)
    )
    ...
    if is_saturday:
        ...
        total_joined = sum(
            len(categories.get(c, [])) for c in ["3限終わり", "4限終わり", "5限終わり"]
        )
    draw.text(
        (40, 90),
        f"日付: {today_str}   |   合計参加: {total_joined}人",
        fill=(224, 231, 255),
        font=small_font,
    )
    ...
    for col in display_order:
        ...
        try:
            draw.rounded_rectangle(
                [card_margin_x, current_y, card_margin_x + 8, current_y + 40], radius=4, fill=color
            )
        except AttributeError:
            draw.rectangle(
                [card_margin_x, current_y, card_margin_x + 8, current_y + 40], fill=color
            )

        # Category Title
        draw.text(
            (card_margin_x + 28, current_y + 2),
            f"{col} ({len(users)}人)",
            fill=(31, 41, 55),
            font=category_font,
        )
        ...
        if not users:
            draw.text(
                (card_margin_x + 28, current_y + 8), "なし", fill=(156, 163, 175), font=text_font
            )
            ...
            for name in users:
                ...
                try:
                    draw.rounded_rectangle(
                        [card_margin_x + 20, current_y, width - card_margin_x, current_y + 48],
                        radius=8,
                        fill=(255, 255, 255),
                    )
                except AttributeError:
                ...
                # User Name Text
                draw.text(
                    (card_margin_x + 40, current_y + 8), name, fill=(75, 85, 99), font=text_font
                )
                current_y += line_height
