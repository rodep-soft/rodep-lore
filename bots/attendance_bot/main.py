...
async def check_meeting_schedule():
    ...
    for meeting in meetings:
        ...
        target_channel_ids = meeting.get("channel_ids") or (
            [meeting.get("channel_id")] if meeting.get("channel_id") else [default_ch_id]
        )
    ...
    cleanup_keys = [
        k
        for k in sent_meeting_reminders
        if (
            now - datetime.datetime.strptime(k[2], "%Y-%m-%d %H:%M").replace(tzinfo=JST)
        ).total_seconds()
        > 7200
    ]
...
async def send_meeting_announcement(
    ...
    try:
        ...
        m_start_time = (
            meeting.get("temp_start_time") or start_time or meeting.get("start_time", "18:00")
        )
        m_agenda = (
            meeting.get("temp_agenda")
            if meeting.get("temp_agenda") is not None
            else (agenda if agenda is not None else meeting.get("agenda", ""))
        )
        m_notice = (
            meeting.get("temp_notice")
            if meeting.get("temp_notice") is not None
            else (notice if notice is not None else meeting.get("notice", ""))
        )

        guild = getattr(channel, "guild", None)
        if guild and mention:
            target_role = None
            clean_name = mention.replace("<@&", "").replace(">", "").lstrip("@").strip()
            # 1. Look for role ID if numeric
            ...
            # 3. Look for case-insensitive match
            if not target_role:
        ...
        lines.append(
            f"参加する人は{attend_emoji} ，参加しない人は{absent_emoji} のリアクションお願いします．"
        )
    ...
    if message.channel.id in target_channels and message.guild:
        ...
        if role and role in message.author.roles:
            try:
                ...
                print(
                    f"Failed to remove '{unanswered_role_name}' role from {message.author.display_name}: {e}"
                )
...
async def send_attendance_message(channel, is_test=False):
    ...
    question = (
        msg_cfg.get("saturday_question", "会議に参加しますか？")
        if is_saturday
        else msg_cfg.get("weekday_question", "今日の活動に参加しますか？")
    )
    note = msg_cfg.get(
        "note",
    ...
    await channel.send(f"{prefix}{mention_text}{countdown_text}{question}{note}", view=view)
...
async def test_send_command(ctx):
    ...
    async with db.pool.acquire() as conn:
        await conn.execute(
            "DELETE FROM attendance WHERE target_date = $1 AND is_test = TRUE", today
        )
...
async def remind_now_command(ctx):
    ...
    if has_unanswered:
        ...
        await channel.send(f"{remind_title}\n{role.mention}\n{remind_body}")
...
async def test_remind_command(ctx):
    ...
    try:
        ...
        await ctx.send(
            f"❌ ロール付与に失敗しました（Botより高い権限の役職を持っている可能性があります）: {e}"
        )
...
async def forget_me_command(ctx):
    ...
    await ctx.send(
        f"👋 {ctx.author.display_name} さんのトラッキングを停止しました。次回以降の集計対象から除外されます。"
    )
...
async def set_event_command(ctx, date_str: str, *, event_name: str):
    ...
    try:
        ...
        await ctx.send(
            "❌ 日付の形式が正しくありません。 `YYYY/MM/DD` で指定してください。（例: 2026/08/15 カンファレンス）"
        )
        return
...
async def set_status_command(ctx, member: discord.Member = None, status: str = None):
    ...
    if member is None or status is None:
        await ctx.send(
            "❌ 形式が正しくありません。 `!set_status @ユーザー 状態` で指定してください。\n(状態例: `3限終わり`, `4限終わり`, `5限終わり`, `出席`, `欠席`)"
        )
        return
...
@commands.has_permissions(manage_messages=True)
async def meeting_add_subcommand(
    ctx, name: str, day_str: str, start_time_str: str = "18:00", *, location_str: str = "研究棟"
):
...
async def meeting_remove_subcommand(ctx, *, name: str):
    ...
    if len(new_meetings) == len(meetings):
        await ctx.send(
            f"❌ 定例会「{name}」は見つかりませんでした。`!meeting list` で登録名を確認してください。"
        )
        return
...
async def meeting_send_subcommand(ctx, *, args: str = ""):
    ...
    target_channel_ids = meeting.get("channel_ids") or (
        [meeting.get("channel_id")] if meeting.get("channel_id") else []
    )
...
async def meeting_location_subcommand(ctx, *, args: str):
    ...
    if is_save:
        ...
        if save_config(cfg):
            await ctx.send(
                f"💾 「**{name}**」の開催場所を 「**{location_str}**」 に**永続保存**しました！"
            )
        else:
            await ctx.send("❌ 設定の保存に失敗しました。")
    else:
        ...
        if save_config(cfg):
            await ctx.send(
                f"⏱️ 「**{name}**」の次回開催場所を 「**{location_str}**」 に**一時変更**しました！（次回送信後に自動リセット）"
            )
        else:
            await ctx.send("❌ 設定の保存に失敗しました。")
...
async def meeting_agenda_subcommand(ctx, *, args: str):
    ...
    if is_save:
        ...
        if save_config(cfg):
            await ctx.send(
                f"💾 「**{name}**」の次回以降の議題を以下のように**永続保存**しました:\n```\n{formatted_agenda}\n```"
            )
        else:
            await ctx.send("❌ 設定の保存に失敗しました。")
    else:
        ...
        if save_config(cfg):
            await ctx.send(
                f"⏱️ 「**{name}**」の次回議題を以下のように**一時変更**しました！（次回送信後に自動リセット）:\n```\n{formatted_agenda}\n```"
            )
        else:
            await ctx.send("❌ 設定の保存に失敗しました。")
...
async def meeting_notice_subcommand(ctx, *, args: str):
    ...
    if is_save:
        ...
        if save_config(cfg):
            await ctx.send(
                f"💾 「**{name}**」の次回以降の連絡事項を以下のように**永続保存**しました:\n```\n{notice_str}\n```"
            )
        else:
            await ctx.send("❌ 設定の保存に失敗しました。")
    else:
        ...
        if save_config(cfg):
            await ctx.send(
                f"⏱️ 「**{name}**」の次回連絡事項を以下のように**一時変更**しました！（次回送信後に自動リセット）:\n```\n{notice_str}\n```"
            )
        else:
            await ctx.send("❌ 設定の保存に失敗しました。")
...
async def meeting_day_subcommand(ctx, *, args: str):
    ...
    if is_save:
        ...
        if save_config(cfg):
            await ctx.send(
                f"💾 「**{name}**」の開催曜日を **{target_day}** に**永続保存**しました！"
            )
        else:
            await ctx.send("❌ 設定の保存に失敗しました。")
    else:
...
async def meeting_time_subcommand(ctx, *, args: str):
    ...
    if is_save:
        ...
        if save_config(cfg):
            await ctx.send(
                f"💾 「**{name}**」の開始時間を **{formatted_time}** に**永続保存**しました！"
            )
        else:
            await ctx.send("❌ 設定の保存に失敗しました。")
    else:
        ...
        if save_config(cfg):
            await ctx.send(
                f"⏱️ 「**{name}**」の次回開始時間を **{formatted_time}** に**一時変更**しました！（次回送信後に自動リセット）"
            )
        else:
            await ctx.send("❌ 設定の保存に失敗しました。")
...
async def list_members_command(ctx):
    ...
    async with db.pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT user_id, name, is_main, is_tracking FROM users ORDER BY is_main DESC, name ASC"
        )
