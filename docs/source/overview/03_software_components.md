...
ステッピングモータというのもあります。これは指令した分だけ決まった角度で動くモータです。これは位置決めしやすいですがフィードバックではなくオープンループ(結果を確認せずに指令だけで動かす)です。一般的にサーボより安いですが、負荷がかかるとズレる(脱調)してしまいます。高速にもあまり強くありません。周波数を変えることで音が変わるのでyoutubeで探したらステッピングモータで音楽作ってる人もいます。
