...
async def help_command(ctx):
    ...
    help_text = (
        "**📋 出欠Bot コマンド一覧**\n\n"
        "👤 **メンバー管理**\n"
        "┣ `!add_member @メンション` : 指定メンバーを集計対象に追加\n"
        "┣ `!remove_member @メンション` : 指定メンバーを集計対象から外す\n"
        "┣ `!list_members` : 現在の集計対象メンバー一覧\n"
        "┗ `!forget_me` : 自分自身を集計対象から外す\n\n"
        "📅 **イベント・カウントダウン**\n"
        "┣ `!set_event YYYY/MM/DD 名前` : イベント追加 (例: 2026/08/10 大会)\n"
        "┣ `!list_events` : 登録済みのイベント一覧\n"
        "┗ `!delete_event 名前` : 指定したイベントを削除\n\n"
        "⚙️ **Botの制御**\n"
        "┣ `!pause` / `!resume` : 毎日の自動送信(8時/12時)を 停止 / 再開\n"
        "┗ `!status` : 現在の自動送信の稼働状況\n\n"
        "🛠️ **テスト・その他**\n"
